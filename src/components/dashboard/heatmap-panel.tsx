...
export function HeatmapPanel({ path, className }: { path: string | null; className?: string }) {
  ...
  const { data, loading, error } = useActivityBuckets(path, HEATMAP_DAYS, "day");
    ...
    <Card className={className}>
      <CardContent className="flex h-full flex-col gap-4">
        <div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-3">
          <PanelValue
            value={data ? streaks.activeDays.toLocaleString(locale) : "—"}
          ...
          <div className="flex items-center gap-6">
            <HeatStat
              ...
              value={
                data ? t("dashboard.heatmap.days", { count: streaks.currentStreak }) : "—"
              }
            />
            <HeatStat
              label={t("dashboard.heatmap.longestStreak")}
              value={
                data ? t("dashboard.heatmap.days", { count: streaks.longestStreak }) : "—"
              }
            />
            <HeatStat
        ...
        {loading && !data ? (
          ...
          <PanelError message={t("dashboard.loadError")} className="h-[116px]" />
        ) : (
          <CalendarHeatmap
            days={heatDays}
            locale={locale}
