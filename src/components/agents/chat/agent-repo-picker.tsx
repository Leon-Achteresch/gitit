...
import {
  useAgentRepoPaths,
  useAgentRepoStore,
} from "@/lib/agents/agent-repo-store";
import {
...
export function AgentRepoPicker({ selectedPath }: { selectedPath: string }) {
  ...
  const worktreeEntries = Object.values(worktrees).sort(
  ...
  const mergeWorktree = async (path: string) => {
    ...
    try {
      ...
      toast.success(
        t("agentChat.worktreeLanded", { name: worktreeDisplayName(path) }),
      );
  ...
  const deleteWorktree = async (path: string) => {
    try {
      ...
      if (
        !window.confirm(
          t("agentChat.worktreeForceConfirm", {
            name: worktreeDisplayName(path),
          }),
        )
      )
        return;
    ...
    if (selectedPath === path)
      setPath(basePath === path ? repoPaths[0] ?? "" : basePath);
  };
            ...
            <>
              ...
              {worktreeEntries.map((entry) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    aria-label={t("agentChat.landWorktree", {
