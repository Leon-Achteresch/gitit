  ...
  return (
    <Dialog open={picker !== null} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md gap-3 p-4">
        ...
        {picker?.detail ? (
          <pre className="rounded-[var(--ag-r-md)] bg-[var(--ag-surface-2)] max-h-56 overflow-auto whitespace-pre-wrap p-2.5 font-mono text-[0.6875rem] leading-4">
            {picker.detail}
          </pre>
        ) : null}
        {picker?.items?.length ? (
          <div className="[scrollbar-color:color-mix(in_oklab,var(--foreground)_16%,transparent)_transparent] [scrollbar-width:thin] max-h-80 overflow-y-auto">
            {picker.items.map((item) => (
              ...
              >
                <span className="min-w-0 flex-1 truncate text-[0.8125rem]">{item.label}</span>
                {item.description ? (
                  <span className="text-[var(--ag-text-3)] min-w-0 max-w-[46%] truncate text-[0.6875rem]">
                    {item.description}
                  </span>
                ) : null}
              </button>
