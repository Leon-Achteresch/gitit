        ...
        return (
          <li key={file.path}>
            ...
            >
              ...
              <span className="relative z-[1] min-w-0 flex-1 truncate font-mono text-[0.6875rem] text-[var(--ag-text)]">
                {file.path}
              </span>
              {accepted.has(file.path) ? (
                <Check className="relative z-[1] size-3 shrink-0 text-git-added" />
              ) : null}
              <span className="relative z-[1] shrink-0 tabular-nums text-[0.625rem]">
                {file.binary ? (
