...
export function FileDiff({
  ...
  const shouldHighlight = currentOpen && !streaming;
  const code = shouldHighlight ? lines.map((line) => line.content).join("\n") : "";
  const tokens = useAgentCodeTokens(code, language, shouldHighlight);
          ...
          <div className="border-[var(--ag-line)] border-t bg-[var(--ag-surface-3)]">
            ...
            >
              <div className="font-mono text-xs leading-5">
                ...
                {currentOpen ? lines.map((line, index) => {
